...
public abstract class HasSecondMagicNumberAbstractCard extends AbstractModCard {
    ...
    public boolean secondMagicNumberModified; // A boolean to check whether the number has been modified or not, for coloring purposes. (red/green)

    public HasSecondMagicNumberAbstractCard(final String id,
                               final String img,
                               final int cost,
                               final CardType type,
    ...
    public void displayUpgrades() { // Display the upgrade - when you click a card to upgrade it
        ...
        if (upgradedSecondMagicNumber) { // If we set upgradedDefaultSecondMagicNumber = true in our card.
            ...
            secondMagicNumberModified = true; // Modified = true, color it green to highlight that the number is being changed.
        }

    }

    public void upgradeBaseSecondMagicNumber(int amount) { // If we're upgrading (read: changing) the number. Note "upgrade" and NOT "upgraded" - 2 different things. One is a boolean, and then this one is what you will usually use - change the integer by how much you want to upgrade.
